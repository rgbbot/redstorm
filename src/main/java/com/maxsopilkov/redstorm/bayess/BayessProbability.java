...
public class BayessProbability {
    ...
    public static double[] run(List<Country> countries) {
        ...
        Country currentCountry = countries.get(0);

        //TODO: Do for current country, check if it makes forecast, do for all countries, dump forecast values into DB.

        evidence.put(conflicts, BayessEvidenceAdapter.fetchConflicts(currentCountry.getConflicts()));
        evidence.put(resources, BayessEvidenceAdapter.fetchResources(currentCountry.getResources()));
        evidence.put(gdp, BayessEvidenceAdapter.fetchGDP(currentCountry.getGdp()));
        evidence.put(nuclear, BayessEvidenceAdapter.fetchNuclear(currentCountry.getNuclear()));
